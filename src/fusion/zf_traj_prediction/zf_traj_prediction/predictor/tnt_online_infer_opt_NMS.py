...
class TNTOnlineInfer:
    ...
    def infer_with_mdn(self, data):
        ...
        self_array = self_array[self_array.any(axis=1)][:,:2]
    ...
    def pairwise_distance_metric(self, traj_batch: torch.Tensor):
        """
        compute pairwise trajectory distance matrix for one batch
        :param traj_batch: torch.Tensor, [M, horizon * 2]
        :return: distance matrix, torch.Tensor, [M, M]
        """
        traj_xy = traj_batch.view(traj_batch.size(0), self.horizon, 2)
        diff = traj_xy.unsqueeze(1) - traj_xy.unsqueeze(0)
        dis = torch.sum(torch.pow(diff, 2), dim=3)
        dis, _ = torch.max(dis, dim=2)
        return dis

    def traj_selection(self, traj_in, score, threshold=16, max_rounds=12):
        """
        ...
        for batch_id in range(traj_pred.shape[0]):  # one batch for a time
            pairwise_dis = self.pairwise_distance_metric(traj_pred[batch_id])
            traj_cnt = 1
            thres = threshold
            rounds = 0
            selected_mask = torch.zeros(self.m, dtype=torch.bool, device=traj_pred.device)
            selected_mask[0] = True
            selected_idx = [0]
            while traj_cnt < self.k and rounds < max_rounds:
                for j in range(1, self.m):
                    if selected_mask[j]:
                        continue
                    dis = pairwise_dis[j, selected_idx]
                    if not torch.any(dis < thres):
                        traj_selected[batch_id, traj_cnt] = traj_pred[batch_id, j].clone()
                        score_selected[batch_id, traj_cnt] = score_pred[batch_id, j]
