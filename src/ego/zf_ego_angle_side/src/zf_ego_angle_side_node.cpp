#include "zf_ego_angle_side/zf_ego_angle_side_node.h"
...
EgoAngleSide::EgoAngleSide(const rclcpp::NodeOptions &options, std::string name)
    : Node(name, options),
      m_fAngleDetected(0.0f),
      m_fAngleFusion(0.0f) {
  m_postfix = this->declare_parameter("cam_ns", "cam1");  // side
  ...
  m_subVehicleInfo = this->create_subscription<VehicleInfoMsg>(
      "/vehicle_info", qos,
      std::bind(&EgoAngleSide::VehicleInfoCallback, this,
                std::placeholders::_1));
  ...
  // std::thread thread1(&EgoAngleSide::InitReceiver, this, 0);
  // thread1.detach();
}

EgoAngleSide::~EgoAngleSide() {}

// TODO
void EgoAngleSide::CalAngle(cv::Mat &img, const uint64_t &timestamp) {
  m_bEabled = true;
  m_fAngleDetected = 10.0;
  m_fConfidence = 0.8;
}

void EgoAngleSide::DetectCbk(cv::Mat &img, uint64_t timestamp) {
  ...
  cv::putText(m_img, "side confidence: ", cv::Point(1710, 150),
  ...
  cv::putText(m_img, "side deg: ", cv::Point(1710, 250),
              cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(147, 20, 255), 2, 4);
...
void EgoAngleSide::VehicleInfoCallback(
...
void EgoAngleSide::NodeStatusCbk() {
...
void EgoAngleSide::InitNodeStatusPublisher() {
...
int main(int argc, char **argv) {
  ...
  auto node = std::make_shared<NS_ZF_DETECTION::EgoAngleSide>(options,
                                                              "ego_angle_side");
  /* 运行节点，并检测退出信号*/
  rclcpp::spin(node);
  rclcpp::shutdown();
