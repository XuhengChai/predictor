                                  ...
                                  WritableBlock *writable_block)
{
  ...
  if (!result)
  {
    LOG_ERROR() << "segment update failed.";
    return false;
  }
...
bool Segment::AcquireBlockToRead(ReadableBlock *readable_block)
{
  ...
  if (!init_ && !OpenOnly())
  {
    LOG_ERROR() << "failed to open shared memory, can't read now.";
    return false;
  }
  ...
  if (index >= conf_.block_num())
  {
    LOG_ERROR() << "invalid block_index[" << index << "].";
    return false;
  }
...
{
  LOG_DEBUG() << "Segment begin destroy." << init_;
  // Reset();// useful if can not delete shm
  if (!init_)
  {
    // return Remove(); // useful if can not delete shm
    return true;
  }
  init_ = false;
