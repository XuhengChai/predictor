...
void ShmConf::Update(const uint64_t &real_msg_size)
{
  ...
  block_buf_size_ = GetBlockBufSize(ceiling_msg_size_);//msg size + msg info size
  block_num_ = GetBlockNum(ceiling_msg_size_);
  managed_shm_size_ =
      EXTRA_SIZE + STATE_SIZE + (RWLOCK_SIZE + block_buf_size_) * block_num_;
}

const uint64_t ShmConf::EXTRA_SIZE = 1024 * 4;
const uint64_t ShmConf::STATE_SIZE = 1024;
const uint64_t ShmConf::RWLOCK_SIZE = 1024;
const uint64_t ShmConf::MESSAGE_INFO_SIZE = 1024;

const uint32_t ShmConf::BLOCK_NUM_1K = 4096;
const uint64_t ShmConf::MESSAGE_SIZE_1K = 1024;
...
uint64_t ShmConf::GetCeilingMessageSize(const uint64_t &real_msg_size)
{
  ...
  if (real_msg_size <= MESSAGE_SIZE_1K)
  {
    ceiling_msg_size = MESSAGE_SIZE_1K;
  }
  else if (real_msg_size <= MESSAGE_SIZE_16K)
  {
...
uint32_t ShmConf::GetBlockNum(const uint64_t &ceiling_msg_size)
{
  ...
  case MESSAGE_SIZE_MORE:
    ...
    break;
  default:
    LOG_ERROR() << "unknown ceiling_msg_size[" << ceiling_msg_size << "]";
    break;
  }
  return num;
}
