...
#ifndef ZF_TRANSPORT_DISPATCHER_INTRA_DISPATCHER_H_
#define ZF_TRANSPORT_DISPATCHER_INTRA_DISPATCHER_H_
...
class ChannelChain {
 ...
 public:
  ...
  bool AddListener(uint64_t self_id, uint64_t channel_id,
    ...
    if (handler == nullptr) {
      LOG_ERROR() << "get handler failed. channel: "
 ...
 private:
  ...
  std::pair<std::shared_ptr<ListenerHandler<MessageT>>, bool> GetHandler(
    ...
    if ((*handlers)[channel_id].find(message_type) ==
      ...
      LOG_DEBUG() << "Create new ListenerHandler for channel "
             << GlobalData::GetChannelById(channel_id)
             << ", message type: " << message_type;
      ...
      LOG_DEBUG() << "Find channel " << GlobalData::GetChannelById(channel_id)
  ...
  ListenerHandlerBasePtr RemoveHandler(int64_t channel_id,
    ...
    if (handlers->find(channel_id) != handlers->end()) {
      if ((*handlers)[channel_id].find(message_type) !=
        ...
        LOG_DEBUG() << "remove " << GlobalData::GetChannelById(channel_id) << "'s "
               << message_type << " ListenerHandler";
        (*handlers)[channel_id].erase(message_type);
      }
      if ((*handlers)[channel_id].empty()) {
        LOG_DEBUG() << "remove " << GlobalData::GetChannelById(channel_id)
  ...
  void Run(const uint64_t channel_id, const std::string& message_type,
    ...
    if (channel_handlers_itr == handlers.end()) {
      LOG_ERROR() << "Cant find channel " << GlobalData::GetChannelById(channel_id)
             << " in Chain";
      return;
    }
    const auto& channel_handlers = channel_handlers_itr->second;

    LOG_DEBUG() << GlobalData::GetChannelById(channel_id)
    ...
    for (const auto& ele : channel_handlers) {
      ...
      if (message_type == ele.first) {
        LOG_DEBUG() << "Run handler for message type: " << ele.first << " directly";
        auto handler =
        ...
        LOG_DEBUG() << "Run handler for message type: " << ele.first
               << " from string";
        if (msg.empty()) {
          auto msg_size = message::FullByteSize(*message);
          if (msg_size < 0) {
            LOG_ERROR() << "Failed to get message size. channel["
          ...
          if (!message::SerializeToHC(*message, const_cast<char*>(msg.data()),
                                      msg_size)) {
            LOG_ERROR() << "Chain Serialize error for channel id: " << channel_id;
            msg.clear();
          }
        }
        if (!msg.empty()) {
...
void IntraDispatcher::OnMessage(uint64_t channel_id,
  ...
  LOG_DEBUG() << "intra on message, channel:"
         << common::GlobalData::GetChannelById(channel_id);
  if (msg_listeners_.Get(channel_id, &handler_base)) {
    ...
    if (handler) {
      ...
      if (message::SerializeToHC(*message, const_cast<char*>(msg.data()),
        ...
        LOG_ERROR() << "Failed to serialize message. channel["
               << common::GlobalData::GetChannelById(channel_id) << "]";
      }
    }
  }
...
std::shared_ptr<ListenerHandler<MessageT>> IntraDispatcher::GetHandler(
  ...
  if (msg_listeners_.Get(channel_id, &handler_base)) {
    handler =
        std::dynamic_pointer_cast<ListenerHandler<MessageT>>(*handler_base);
    if (handler == nullptr) {
      LOG_DEBUG() << "Find a new type for channel "
...
void IntraDispatcher::AddListener(const RoleAttributes& self_attr,
  ...
  auto channel_id = self_attr.GetChannelId();
  ...
  auto handler = GetHandler<MessageT>(self_attr.GetChannelId());
  if (handler && created) {
...
void IntraDispatcher::RemoveListener(const RoleAttributes& self_attr) {
  ...
  chain_->RemoveListener<MessageT>(self_attr.id(), self_attr.GetChannelId(),
                                   message::GetMessageName<MessageT>());
}
...
void IntraDispatcher::RemoveListener(const RoleAttributes& self_attr,
  ...
  chain_->RemoveListener<MessageT>(self_attr.id(), opposite_attr.id(),
                                   self_attr.GetChannelId(),
                                   message::GetMessageName<MessageT>());
}

END_NS_ZF_FRAMEWORK

#endif  // ZF_TRANSPORT_DISPATCHER_INTRA_DISPATCHER_H_
