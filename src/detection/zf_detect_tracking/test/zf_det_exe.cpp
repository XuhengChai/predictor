...
// using namespace NS_ZF_DETECTION;

// int main(int argc, char ** argv)
// {
//   (void) argc;
//   (void) argv;
//   DetcetEgoAngle m_pDetectedAngle;
//   zf::LOG_DEBUG() << "m_pDetectedAngle " << &m_pDetectedAngle;
...
void inference(){
    ...
    TRTLogger logger;
    zf::LOG_DEBUG() << "createInferRuntime ";

    // auto engine_data = load_file("engine.trtmodel");
    // 执行推理前，需要创建一个推理的runtime接口实例。与builer一样，runtime需要logger：
    nvinfer1::IRuntime* runtime   = nvinfer1::createInferRuntime(logger);
        zf::LOG_DEBUG() << "build_model done";
